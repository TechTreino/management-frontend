...
abstract class Customer implements ActiveRecordInterface
{
    ...
    /**
     * @var        ObjectCollection|ChildTraining[] Collection to store aggregation of ChildTraining objects.
     */
    protected $collTrainings;
    protected $collTrainingsPartial;
     ...
     * @var ObjectCollection|ChildTraining[]
     */
    ...
    public function setModifiedAt($v)
    {
        ...
        if ($this->modified_at !== null || $dt !== null) {
            if ($this->modified_at === null || $dt === null || $dt->format("Y-m-d H:i:s.u") !== $this->modified_at->format("Y-m-d H:i:s.u")) {
                ...
                $this->modifiedColumns[CustomerTableMap::COL_MODIFIED_AT] = true;
            }
        } // if either are not null
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 4 + $startcol : CustomerTableMap::translateFieldName('Password', TableMap::TYPE_PHPNAME, $indexType)];
            $this->password = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 5 + $startcol : CustomerTableMap::translateFieldName('CreatedAt', TableMap::TYPE_PHPNAME, $indexType)];
            if ($col === '0000-00-00 00:00:00') {
                $col = null;
            }
            $this->created_at = (null !== $col) ? PropelDateTime::newInstance($col, null, 'DateTime') : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 6 + $startcol : CustomerTableMap::translateFieldName('ModifiedAt', TableMap::TYPE_PHPNAME, $indexType)];
            if ($col === '0000-00-00 00:00:00') {
                $col = null;
            }
    ...
    protected function doSave(ConnectionInterface $con)
    {
        ...
        if (!$this->alreadyInSave) {
            ...
            if ($this->collTrainings !== null) {
                foreach ($this->collTrainings as $referrerFK) {
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(CustomerTableMap::COL_MODIFIED_AT)) {
    ...
    public function toArray($keyType = TableMap::TYPE_PHPNAME, $includeLazyLoadColumns = true, $alreadyDumpedObjects = array(), $includeForeignObjects = false)
    {
        ...
        if ($includeForeignObjects) {
            if (null !== $this->collTrainings) {
                ...
                $result[$key] = $this->collTrainings->toArray(null, false, $keyType, $includeLazyLoadColumns, $alreadyDumpedObjects);
            }
        }
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(CustomerTableMap::COL_MODIFIED_AT)) {
            $criteria->add(CustomerTableMap::COL_MODIFIED_AT, $this->modified_at);
        }
    ...
    public function copyInto($copyObj, $deepCopy = false, $makeNew = true)
    {
        ...
        if ($deepCopy) {
            ...
            foreach ($this->getTrainings() as $relObj) {
                if ($relObj !== $this) {  // ensure that we don't try to copy a reference to ourselves
                    $copyObj->addTraining($relObj->copy($deepCopy));
                }
            }
    ...
    {
        if ('Training' == $relationName) {
            return $this->initTrainings();
        }
    }

    /**
     * Clears out the collTrainings collection
     *
    ...
    {
        $this->collTrainings = null; // important to set this to NULL since that means it is uninitialized
    }

    /**
     * Reset is the collTrainings collection loaded partially.
     */
    public function resetPartialTrainings($v = true)
    {
        $this->collTrainingsPartial = $v;
    }

    /**
     * Initializes the collTrainings collection.
     *
     * By default this just sets the collTrainings collection to an empty array (like clearcollTrainings());
     ...
     * @return void
     */
    public function initTrainings($overrideExisting = true)
    {
        if (null !== $this->collTrainings && !$overrideExisting) {
            return;
        }
    ...
    /**
     * Gets an array of ChildTraining objects which contain a foreign key that references this object.
     *
     ...
     * @return ObjectCollection|ChildTraining[] List of ChildTraining objects
     * @throws PropelException
     */
    public function getTrainings(Criteria $criteria = null, ConnectionInterface $con = null)
    {
        $partial = $this->collTrainingsPartial && !$this->isNew();
        if (null === $this->collTrainings || null !== $criteria  || $partial) {
            if ($this->isNew() && null === $this->collTrainings) {
                // return empty collection
                $this->initTrainings();
            } else {
                $collTrainings = ChildTrainingQuery::create(null, $criteria)
                    ->filterByCustomer($this)
                    ->find($con);

                if (null !== $criteria) {
                    if (false !== $this->collTrainingsPartial && count($collTrainings)) {
                        $this->initTrainings(false);

                        foreach ($collTrainings as $obj) {
                            if (false == $this->collTrainings->contains($obj)) {
                                $this->collTrainings->append($obj);
                            }
                        }

                        $this->collTrainingsPartial = true;
                    }

                    return $collTrainings;
                }

                if ($partial && $this->collTrainings) {
                    foreach ($this->collTrainings as $obj) {
                        if ($obj->isNew()) {
                            $collTrainings[] = $obj;
                        }
                    }
                ...
                $this->collTrainings = $collTrainings;
                $this->collTrainingsPartial = false;
            }
        }

        return $this->collTrainings;
    }

    /**
     * Sets a collection of ChildTraining objects related by a one-to-many relationship
     * to the current object.
     * It will also schedule objects for deletion based on a diff between old objects (aka persisted)
     * and new objects from the given Propel collection.
     *
     * @param      Collection $trainings A Propel collection.
     * @param      ConnectionInterface $con Optional connection object
     * @return $this|ChildCustomer The current object (for fluent API support)
     */
    public function setTrainings(Collection $trainings, ConnectionInterface $con = null)
    {
        /** @var ChildTraining[] $trainingsToDelete */
        $trainingsToDelete = $this->getTrainings(new Criteria(), $con)->diff($trainings);


        $this->trainingsScheduledForDeletion = $trainingsToDelete;

        foreach ($trainingsToDelete as $trainingRemoved) {
            $trainingRemoved->setCustomer(null);
        }
    ...
    /**
     * Returns the number of related Training objects.
     *
     * @param      Criteria $criteria
     * @param      boolean $distinct
     * @param      ConnectionInterface $con
     * @return int             Count of related Training objects.
     * @throws PropelException
     */
    public function countTrainings(Criteria $criteria = null, $distinct = false, ConnectionInterface $con = null)
    {
        $partial = $this->collTrainingsPartial && !$this->isNew();
        if (null === $this->collTrainings || null !== $criteria || $partial) {
            if ($this->isNew() && null === $this->collTrainings) {
            ...
            if ($partial && !$criteria) {
                return count($this->getTrainings());
            }

            $query = ChildTrainingQuery::create(null, $criteria);
            if ($distinct) {
                $query->distinct();
            }
        ...
        return count($this->collTrainings);
    }

    /**
     * Method called to associate a ChildTraining object to this object
    ...
    public function addTraining(ChildTraining $l)
    {
        ...
        if (!$this->collTrainings->contains($l)) {
            $this->doAddTraining($l);

            if ($this->trainingsScheduledForDeletion and $this->trainingsScheduledForDeletion->contains($l)) {
                $this->trainingsScheduledForDeletion->remove($this->trainingsScheduledForDeletion->search($l));
            }
        }
    ...
    /**
     * @param ChildTraining $training The ChildTraining object to add.
     */
    protected function doAddTraining(ChildTraining $training)
    {
        $this->collTrainings[]= $training;
        $training->setCustomer($this);
    }

    /**
     * @param  ChildTraining $training The ChildTraining object to remove.
     * @return $this|ChildCustomer The current object (for fluent API support)
     */
    public function removeTraining(ChildTraining $training)
    {
        if ($this->getTrainings()->contains($training)) {
            $pos = $this->collTrainings->search($training);
            $this->collTrainings->remove($pos);
            if (null === $this->trainingsScheduledForDeletion) {
                $this->trainingsScheduledForDeletion = clone $this->collTrainings;
                $this->trainingsScheduledForDeletion->clear();
            }
            $this->trainingsScheduledForDeletion[]= clone $training;
    ...
    {
        if ($deep) {
            if ($this->collTrainings) {
                foreach ($this->collTrainings as $o) {
